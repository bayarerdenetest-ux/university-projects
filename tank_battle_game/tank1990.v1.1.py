...
#-----New features--------------------------------------------------------------
# In this version we added three levels (easy/ normal/ hard), fire sound, and now players can choose the number of bots. 
...
# ─── Difficulty settings ──────────────────────────────────────────────────────
DIFFICULTY = {
    'easy':   {'bot_think': (60,120), 'bot_shoot_chance': 0.002, 'bot_speed_mul': 0.7,
               'shoot_cd': 35, 'chase_dist': 5,  'label': 'EASY',   'color': C_GREEN},
    'normal': {'bot_think': (30, 80), 'bot_shoot_chance': 0.005, 'bot_speed_mul': 1.0,
               'shoot_cd': 25, 'chase_dist': 8,  'label': 'NORMAL', 'color': C_YELLOW},
    'hard':   {'bot_think': (10, 35), 'bot_shoot_chance': 0.012, 'bot_speed_mul': 1.4,
               'shoot_cd': 14, 'chase_dist': 14, 'label': 'HARD',   'color': C_RED},
}
...
def draw_tank(surf, x, y, direction, color, is_player=True):
    ...
    tread_col  = tuple(max(0,c-60) for c in color)
    barrel_col = tuple(max(0,c-30) for c in color)
    if direction in (0,2):
        pygame.draw.rect(surf, tread_col, (x+2, y+2, 7, TILE-4))
        pygame.draw.rect(surf, tread_col, (x+TILE-9, y+2, 7, TILE-4))
        for i in range(4):
            yy = y+4+i*7
        ...
        if direction==0: pygame.draw.rect(surf, barrel_col, (cx-3, y, 6, 14))
        else:            pygame.draw.rect(surf, barrel_col, (cx-3, y+TILE-14, 6, 14))
    else:
        pygame.draw.rect(surf, tread_col, (x+2, y+2, TILE-4, 7))
        pygame.draw.rect(surf, tread_col, (x+2, y+TILE-9, TILE-4, 7))
        for i in range(4):
            xx = x+4+i*7
        ...
        if direction==1: pygame.draw.rect(surf, barrel_col, (x+TILE-14, cy-3, 14, 6))
        else:            pygame.draw.rect(surf, barrel_col, (x, cy-3, 14, 6))
    if is_player:
    ...
    def update(self, grid):
        ...
        for corner in [(self.x,self.y),(self.x+self.w,self.y),
            ...
            if 0<=tx<COLS and 0<=ty<ROWS:
                t = grid[ty][tx]
                if t==T_BRICK:
                    grid[ty][tx]=T_EMPTY; self.alive=False; return 'brick'
                elif t==T_STEEL:
                    self.alive=False; return 'steel'
                elif t==T_FORT:
                    grid[ty][tx]=T_EMPTY; self.alive=False; return 'fort'
        return False
...
class BotAI:
    ...
    def _line_of_sight(self, grid, targets):
        ...
        for step in range(1, COLS):
            ...
            for tgt in targets:
                if tgt.alive and tgt.rect().collidepoint(cx2,cy2):
                    return True
        return False
    ...
    def update(self, grid, player_tanks, bullets):
        ...
        if self.think_timer <= 0:
            lo,hi = d['bot_think']
            self.think_timer = random.randint(lo, hi)
...
def spawn_explosion(cx, cy, big=False):
    ...
    for _ in range(20 if big else 12):
        ...
        sp= random.uniform(1, 4 if big else 3)
        parts.append({'x':float(cx),'y':float(cy),
...
def draw_hud(surf, font, small_font, game):
    ...
    for pid,color,label,ks in [('p1',C_GOLD,'P1','WASD+F'),('p2',C_SILVER,'P2','ARR+/')]:
        ...
        y+=26
        k=small_font.render(ks,True,C_GRAY)
        surf.blit(k,(ox+12,y)); y+=18
...
# ─── Settings screen (bot count + difficulty) ─────────────────────────────────
class Settings:
    DIFF_ORDER = ['easy','normal','hard']
    MIN_BOTS, MAX_BOTS = 1, 12

    def __init__(self, mode):
        self.mode       = mode    # 1 or 2 players
        self.diff_idx   = 1       # normal default
        self.bot_count  = 6
        self.cursor     = 0       # 0=difficulty, 1=bot_count, 2=start
    ...
    def handle_key(self, key):
        ...
        if key in (pygame.K_UP, pygame.K_w):
            ...
            if self.cursor==0:
                ...
                self.bot_count = max(self.MIN_BOTS, self.bot_count-1)
        elif key in (pygame.K_RIGHT, pygame.K_d):
            if self.cursor==0:
                self.diff_idx = (self.diff_idx+1) % 3
            elif self.cursor==1:
                self.bot_count = min(self.MAX_BOTS, self.bot_count+1)
...
def draw_settings(surf, font, big_font, small_font, settings, blink):
    ...
    surf.blit(title,(WIN_W//2-title.get_width()//2, 60))
    mode_lbl=font.render(f"{'1 PLAYER' if settings.mode==1 else '2 PLAYERS'}", True, C_ORANGE)
    surf.blit(mode_lbl,(WIN_W//2-mode_lbl.get_width()//2, 120))
    ...
    for i,(label,_) in enumerate(items):
        selected = (i == settings.cursor)
        col = C_WHITE if selected else C_GRAY
        ...
        if i==0:
            ...
            bx = WIN_W//2-140
            by = y+34
            arrow_l=font.render("<",True,C_YELLOW if selected else C_GRAY)
            surf.blit(arrow_l,(bx,by))
            cnt=font.render(str(settings.bot_count),True,C_WHITE)
            surf.blit(cnt,(WIN_W//2-cnt.get_width()//2, by))
            arrow_r=font.render(">",True,C_YELLOW if selected else C_GRAY)
            surf.blit(arrow_r,(WIN_W//2+120,by))
            # Visual dots
            for k in range(settings.bot_count):
                dx2=WIN_W//2-settings.bot_count*10+k*20
                pygame.draw.circle(surf,C_RED,(dx2,by+40),7)

    hint=small_font.render("←→ change value    ↑↓ move    Enter confirm    Esc back",True,C_GRAY)
    surf.blit(hint,(WIN_W//2-hint.get_width()//2, WIN_H-40))
...
def draw_menu(surf, font, big_font, small_font, selected, blink):
    ...
    surf.blit(sub,(WIN_W//2-sub.get_width()//2, 132))
    draw_tank(surf, WIN_W//2-120, 175, 2, C_GOLD,   True)
    draw_tank(surf, WIN_W//2+90,  175, 2, C_SILVER, True)
    ...
    for lbl,val,col in [("P1:","WASD move  F shoot",C_GOLD),
        ...
        if lbl:
            surf.blit(small_font.render(lbl,True,col),(WIN_W//2-170,ctrl_y))
        surf.blit(small_font.render(val,True,C_GRAY),(WIN_W//2-100,ctrl_y))
        ctrl_y+=22
...
def draw_gameover(surf, font, big_font, small_font, won, score, blink):
    ...
    surf.blit(t,(WIN_W//2-t.get_width()//2,160))
    sc=font.render(f"Score: {score}",True,C_WHITE)
    surf.blit(sc,(WIN_W//2-sc.get_width()//2,268))
    if blink:
        c=small_font.render("Enter = play again     Esc = menu",True,C_GRAY)
        surf.blit(c,(WIN_W//2-c.get_width()//2,370))

# ─── Game class ───────────────────────────────────────────────────────────────
SPAWN_POS = [(1*TILE,0),(12*TILE,0),(23*TILE,0),
             (6*TILE,0),(18*TILE,0),(3*TILE,0),(21*TILE,0),
             (8*TILE,0),(16*TILE,0),(10*TILE,0),(14*TILE,0),(5*TILE,0)]
...
class Game:
    ...
    def _try_spawn(self):
        ...
        pos=random.choice(SPAWN_POS[:max(3,1)])
        e=Tank(*pos, C_RED,'enemy', spd)
        e.dir=2
        er=e.rect()
        ...
        self.enemies.append(e)
        self.bots.append(BotAI(e, self.diff))
        self.enemy_queue-=1
    ...
    def update(self, keys):
        ...
        if self.spawn_timer<=0:
            self.spawn_timer=180; self._try_spawn()
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # ── Settings screen ──────────────────────────────────────────
                    ...
                    if result == 'start':
                        game  = Game(settings.mode, settings.diff,
                                     settings.bot_count, sounds)
                        state = GameMode.PLAY

                # ── Game over ────────────────────────────────────────────────
                elif state == GameMode.GAMEOVER:
                    if event.key == pygame.K_RETURN:
                        game  = Game(game.mode, game.diff,
                                     settings.bot_count, sounds)
                        state = GameMode.PLAY
        ...
        if state == GameMode.MENU:
            ...
            draw_settings(surf, font, big_font, small_font, settings, blink)
        elif state == GameMode.PLAY and game:
            game.draw(surf)
            ...
            draw_gameover(surf, font, big_font, small_font,
                          game.won, game.score, blink)
